"""
Loads main corpus and generates slices for multiprocessing.

These slices can be used to run analyses in parallel on all cores,
though this is not recommended.

Instead, you should use `segment.py` to generate a representative
sample of the corpus (which works fine for frequency analysis and
similar tasks) and divide that using `divide.py`, which performs an
identical function to this script, but for the representative sample.

Performing analyses on the files generated by this script is about 100x
slower than using the representative sample!
"""
